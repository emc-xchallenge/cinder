...
class EMCVnxCliBase(object):
    ...
    def auto_register_initiator(self, connector, sgdata, io_ports_filter=None):
        ...
        :returns: True if has registered initiator otherwise will throws exception
        """
        ref = False
        if io_ports_filter:
            ref = self.auto_register_with_io_port_filter(connector, sgdata,
                                                          io_ports_filter)
        else:
            ref = self.auto_register_initiator_to_all(connector, sgdata)
